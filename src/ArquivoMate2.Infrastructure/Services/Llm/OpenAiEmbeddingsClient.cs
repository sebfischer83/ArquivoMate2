...
namespace ArquivoMate2.Infrastructure.Services.Llm
{
    ...
    public sealed class OpenAiEmbeddingsClient : IEmbeddingsClient, IDisposable
    {
        ...
        public async Task<IReadOnlyList<float[]>> GenerateEmbeddingsAsync(IReadOnlyList<string> inputs, CancellationToken cancellationToken = default)
        {
            ...
            if (response?.Data == null) return Array.Empty<float[]>();

            var result = response.Data.Select(d => d.Embedding.ToArray()).ToList().AsReadOnly();
            return result;
        }
        ...
        public async Task<float[]> GenerateEmbeddingAsync(string input, CancellationToken cancellationToken = default)
        {
            ...
            return first is null ? Array.Empty<float>() : first.Embedding.ToArray();
        }
